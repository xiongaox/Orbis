...
export function calculateWangShuai(pillars: Array<{ tiangan: string, dizhi: string }>): WangShuaiResult {
    ...
    Object.entries(monthRoots).forEach(([root, weight]) => {
        ...
        const resourceEl = cycle[(idx + 4) % 5];
    ...
    if (isTrueTransformation && transformGodElement) {
        ...
        const getDaySeatDefense = (dmStem: string, dayBranch: string): { isDefense: boolean; reason: string } => {
            ...
            if (mainQi) {
                ...
                if (mainQiInfo && mainQiInfo.el === dmEl) {
                    return { isDefense: true, reason: `日支${dayBranch}本气${mainQiStem}为日主同五行(${dmEl})，坐禄/比/劫，根气强劲，绝不入从` };
                }
            }
        ...
        const { isDefense: daySeatDefense, reason: daySeatReason } = getDaySeatDefense(dmStem, branches[2]); // branches[2] is Day Branch
