...
import { DI_ZHI_CANG_GAN, TIAN_GAN_WU_XING, DI_ZHI_WU_XING } from './constants.ts';
...
export function calculateWangShuai(pillars: Array<{ tiangan: string, dizhi: string }>): WangShuaiResult {
    ...
    Object.entries(monthRoots).forEach(([root, weight]) => {
        ...
        const isResource = supportEls.includes(rootEl!);
        ...
        if (isMonthHostile) {
            ...
            // 月令生身或同气，印星打7折（因为印不如比劫直接）
            if (isResource) finalWeight *= 0.7;
        }
    ...
    // 规则2: 官杀混杂且无制，强力克身
    if (counts[relations['Official']] >= 3 && counts[relations['Output']] === 0) {
        extraPenalty += 1.5;
    ...
    // Round to 2 decimals
    let zScore = (0.1585 * monthScore) + (0.0139 * rootsScore) + (0.0336 * stemsScore) - 2.2463 - extraPenalty + extraBonus;
    if (isEarthDominant) zScore += 5.0;

    // DEBUG: 打印详细分数
    // console.log(`[DEBUG] Month(${monthCorrectionFactor.toFixed(1)}): ${monthScore.toFixed(1)}, Roots: ${rootsScore}, Stems: ${stemsScore}, Extra: -${extraPenalty}, RawZ: ${zScore.toFixed(2)}`);
    // console.log(`[DEBUG] Party: Same=${totalSameParty}, Opp=${totalOppositeParty}`);
