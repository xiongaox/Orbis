 ...
 *
 * 主要导出：
 * - `default Navbar`, `ChartType`
 *
 * 依赖关系：
 * - 上游依赖：外部依赖 `react`、外部依赖 `lucide-react`、内部模块 `useAuth` 等 12 个模块
            ...
            <div className="flex items-center gap-1.5 md:gap-2 lg:gap-4 md:absolute md:right-2 lg:right-6">
              ...
              {!isPadLandscape && (
                <a
                  ...
                  title="GitHub 仓库"
                >
                  <svg viewBox="0 0 200 200" className={`w-5 h-5 ${useDesktopLayout ? 'lg:w-[22px] lg:h-[22px]' : ''} text-muted-foreground`} fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              >
                {isDark ? (
                  <Sun className={`w-4 h-4 ${useDesktopLayout ? 'lg:w-[22px] lg:h-[22px]' : ''} text-muted-foreground`} />
                ) : (
                  <Moon className={`w-4 h-4 ${useDesktopLayout ? 'lg:w-[22px] lg:h-[22px]' : ''} text-muted-foreground`} />
                )}
              </button>
