...
function SimplePillarCard({
    ...
    return (
        <div className={`h-full flex flex-col ${isDayMaster ? 'bg-primary/5' : ''} ${isYunPillar ? 'bg-accent/5' : ''}`}>
            <div className={`h-8 flex items-center justify-center border-b border-border ${isYunPillar ? 'bg-accent/10' : 'bg-secondary/30'}`}>
                ...
                <div className="flex">
                    ...
                    <div className="w-16 flex-shrink-0 border-r border-border flex flex-col">
                        {['日期', '主星', '天干', '地支'].map((label, i) => (
                            <div
                                key={label}
                                className={`${i < 2 ? 'h-8 h-10'.split(' ')[i] : 'h-14'} flex items-center justify-center border-b border-border ${i === 0 ? 'bg-secondary/30' : 'bg-muted/30'}`}
                            >
                        ...
                        {['星运', '自坐', '空亡', '纳音'].map((label, idx) => (
                            <div
                                key={label}
                                className={`h-10 flex items-center justify-center bg-muted/30 ${idx < 3 ? 'border-b border-border' : ''}`}
                            >
                                <span className="text-xs text-muted-foreground">{label}</span>
                            </div>
                        ))}
                    ...
                    {liuNianPillar && (
                        <div className="flex-1 border-r border-border">
                            <SimplePillarCard pillar={liuNianPillar} isYunPillar />
                        </div>
                    )}
                    ...
                    {daYunPillar && (
                        <div className="flex-1 border-r border-border">
                            <SimplePillarCard pillar={daYunPillar} isYunPillar />
                        </div>
                    )}
