...
const METHODS: { value: PaiPanMethod; label: string; shortLabel: string }[] = [
...
export default function CaseStudyQimenHeader({
    ...
    return (
        <div className={`w-full ${isMobile ? 'p-2' : 'p-4'} flex flex-col ${isMobile ? 'gap-2' : 'gap-3'} bg-card border-b border-border`}>
            {/* 第一行：日期时间 */}
            <div className="flex items-center justify-between">
                <div className={`flex items-baseline ${isMobile ? 'gap-1 flex-wrap' : 'gap-2'}`}>
                    <span className={`${isMobile ? 'text-sm' : 'text-lg'} font-serif font-bold text-foreground`}>
                        {header.solarDate.replace(/年|月|日/g, (match) => ` ${match} `)}
                    </span>
                    <span className={`${isMobile ? 'text-xs' : 'text-base'} font-serif text-foreground/80`}>({header.lunarDate})</span>
                    <span className={`${isMobile ? 'text-sm' : 'text-lg'} font-serif font-bold text-foreground ${isMobile ? 'ml-1' : 'ml-2'}`}>{header.time}</span>
                </div>
            </div>

            {/* 第二行：四柱 + 信息 */}
            {header.siZhu?.year && (
                <div className={`flex items-center justify-start ${isMobile ? 'flex-wrap gap-2' : ''}`}>
                    <div className={`flex ${isMobile ? 'gap-2' : 'gap-4'}`}>
                        {(['year', 'month', 'day', 'hour'] as const).map((key) => {
                            ...
                            const isJia = stem === '甲';
                            return (
                                <div key={key} className={`flex flex-col items-center relative ${isMobile ? 'pr-1.5' : 'pr-2.5'}`}>
                                    <span className={`${isMobile ? 'text-base' : 'text-xl'} font-serif leading-tight ${isDayOrHour ? 'text-primary font-bold' : 'text-foreground'}`}>{stem}</span>
                                    <span className={`${isMobile ? 'text-base' : 'text-xl'} font-serif leading-tight ${isDayOrHour && isJia ? 'text-primary font-bold' : 'text-foreground'}`}>{branch}</span>
                                    <span className={`absolute top-1/2 -translate-y-1/2 -right-1.5 ${isMobile ? 'text-[10px]' : 'text-xs'} text-muted-foreground/60 font-serif origin-right`}>{labels[key]}</span>
                                </div>
                            );
                        })}
                    </div>
                        ...
                        {[
                            ...
                            <div key={i} className="flex items-center gap-1.5">
                                ...
                                {item.clickable ? (
                                    <button
                                        onClick={onJuClick}
                                        className={`text-primary font-serif font-bold ${isMobile ? 'text-sm' : 'text-base'} hover:underline underline-offset-2 transition-colors cursor-pointer`}
                                        title="点击自定义局数"
                                    >
                                        {item.value}
                                    </button>
                                ) : (
                                    <span className={`text-foreground font-serif ${isMobile ? 'text-sm' : 'text-base'} ${item.bold ? 'font-bold' : ''}`}>{item.value}</span>
                                )}
                            </div>
                        ))}
                    </div>
            ...
            {/* 第三行：排盘方法 + 标签按钮 */}
            {isMobile ? (
                /* 移动端：排盘方法按钮已移至九宫格下方，这里只保留 globalPatterns + 长生/十神 */
                <div className="flex items-center justify-between pt-1">
                    <div className="flex items-center gap-1">
                        {globalPatterns.map((pattern, idx) => (
                            <button key={idx} onClick={() => onPatternClick?.(pattern)} className="px-2 py-0.5 rounded-md border border-border text-muted-foreground text-[11px] font-serif hover:bg-muted/10 transition-colors" title={pattern.fullLabel}>{pattern.label}</button>
                        ))}
                    </div>
                    <div className="flex items-center gap-1">
                        <button onClick={onToggleChangSheng} className={`px-2 py-0.5 text-[11px] rounded-md transition-colors font-serif border border-border ${showChangSheng ? 'bg-primary/10 text-primary' : 'bg-secondary text-muted-foreground hover:bg-secondary/80'}`}>长生状态</button>
                        <button onClick={onToggleShiShen} className={`px-2 py-0.5 text-[11px] rounded-md transition-colors font-serif border border-border ${showShiShen ? 'bg-amber-500/10 text-amber-500' : 'bg-secondary text-muted-foreground hover:bg-secondary/80'}`}>十神展示</button>
                    </div>
                </div>
            ) : (
                /* 桌面端：保持原有下拉菜单 */
