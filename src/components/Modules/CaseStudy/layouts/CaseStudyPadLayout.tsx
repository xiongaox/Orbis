 ...
 *
 * 主要导出：
 * - `default CaseStudyPadLayout`
 *
 * 依赖关系：
 * - 上游依赖：外部依赖 `lucide-react`、外部依赖 `react-markdown`、外部依赖 `rehype-raw` 等 18 个模块
...
export default function CaseStudyPadLayout(props: CaseStudyLayoutProps) {
    const {
        ...
        <div className="h-full bg-muted/10 flex flex-col overflow-hidden">
                ...
                <div className="p-2 border-b border-border bg-muted/20 flex justify-between items-center h-[40px]">
                    <div className="flex space-x-1">
                        {Array.from({ length: chartCount }).map((_, i) => (
                            <button key={i} onClick={() => setActiveChartIndex(i)} className={`text-[10px] px-2 py-0.5 rounded border transition-colors ${activeChartIndex === i ? 'bg-primary text-primary-foreground border-primary' : 'bg-background text-muted-foreground border-border hover:bg-muted'}`}>
                                盘式{toChineseNum(i + 1)}
                            </button>
                        ))}
                    </div>
            ...
            <div className="flex-1 overflow-y-auto p-0 scrollbar-thin scrollbar-thumb-border/40 scrollbar-track-transparent flex flex-col">
                {selectedCategory === 'bazi' && activeCase ? (
                    ...
                    qimenResult ? (
                        <div className="flex-shrink-0 flex flex-col"><CaseStudyQimenChart palaces={qimenResult.palaces} selectedPalace={null} onSelectPalace={() => { }} header={qimenResult.header} globalPatterns={qimenResult.globalPatterns} onJuClick={() => setIsJuDialogOpen(true)} isMobile={false} method={qimenMethod} onMethodChange={setQimenMethod} /></div>
    ...
    return (
        <>
            ...
            <div className="flex-1 flex flex-col min-h-0 overflow-hidden relative">
                ...
                <button type="button" onClick={() => setIsChartPanelOpen(true)} className="absolute right-0 top-1/2 -translate-y-1/2 z-30 w-8 h-28 bg-transparent flex items-center justify-end group focus:outline-none" aria-label="打开排盘">
                    ...
                    <span className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity"><span className="px-2 py-1 rounded-md text-xs bg-card border border-border shadow-sm text-foreground/80 whitespace-nowrap">排盘</span></span>
                </button>
                {contentColumn}
            </div>
            ...
            <SideDrawer open={isChartPanelOpen} title="排盘信息" side="right" size="lg" onClose={() => setIsChartPanelOpen(false)}>
                {chartPanel}
            </SideDrawer>
        </>
    );
