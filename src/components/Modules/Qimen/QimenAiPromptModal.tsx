        ...
        <BaseModal
            ...
            title={isMobile ? (
                <div className="flex items-center gap-2">
                    <span className="text-lg font-medium text-foreground">奇门信息提示词</span>
                    ...
                    <div className="flex-1 overflow-y-auto p-3 space-y-4">
                        ...
                        <div className="grid grid-cols-3 gap-2">
                            <label className="flex items-center gap-2 px-2 py-2.5 rounded-lg border border-border bg-muted/30 cursor-pointer text-xs">
                                <div className={`w-3.5 h-3.5 rounded border flex items-center justify-center flex-shrink-0 ${includeGlobalPatterns ? 'bg-amber-500 border-amber-500' : 'border-input'}`}>
                                    {includeGlobalPatterns && <Check className="w-2.5 h-2.5 text-black" />}
                                </div>
                                <input type="checkbox" className="sr-only" checked={includeGlobalPatterns} onChange={(e) => setIncludeGlobalPatterns(e.target.checked)} />
                                含格局信息
                            </label>
                            <label className="flex items-center gap-2 px-2 py-2.5 rounded-lg border border-border bg-muted/30 cursor-pointer text-xs">
                                <div className={`w-3.5 h-3.5 rounded border flex items-center justify-center flex-shrink-0 ${includeAllPalaces ? 'bg-amber-500 border-amber-500' : 'border-input'}`}>
                                    {includeAllPalaces && <Check className="w-2.5 h-2.5 text-black" />}
                                </div>
                                <input type="checkbox" className="sr-only" checked={includeAllPalaces} onChange={(e) => setIncludeAllPalaces(e.target.checked)} />
                                含宫位详情
                            </label>
                            <button
                                onClick={handleCopy}
