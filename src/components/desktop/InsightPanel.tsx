...
export default function InsightPanel({ className = '' }: InsightPanelProps) {
  const [activeBook, setActiveBook] = useState('dayun');
  ...
  return (
    ...
    >
      ...
      <div className="p-4 border-b border-border">
        <div className="text-xs text-muted-foreground mb-2">调候用神提示：甲 壬</div>
        <div className="text-xs text-muted-foreground mb-3">本八字：连 壬 亘 甲</div>
        <div className="flex flex-wrap gap-1.5">
          {insightBooks.map((item) => (
            <button
              key={item.id}
              type="button"
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        <div className="bg-secondary/30 rounded-lg border border-border">
          ...
          {openSections.includes('keyPoints') && (
            <div className="px-3 pb-3 space-y-2">
              {insightContent.keyPoints.map((item) => (
        ...
        <div className="bg-secondary/30 rounded-lg border border-border">
          ...
          {openSections.includes('advice') && (
            <div className="px-3 pb-3">
              <p className="text-sm text-muted-foreground leading-relaxed">{insightContent.advice}</p>
            </div>
          )}
        </div>
      </div>
