 ...
 *
 * 主要导出：
 * - `ShenShaContext`, `ShenShaResult`, `calculateShenSha`, `calculateDynamicShenSha`, `isShiEDaBai`, `isYinChaYangCuo`, `isGuLuanSha`, `isLiuXiuRi`, `isShiLingRi`, `isKuiGangRi`, `isBaZhuanRi`, `isJiuChouRi`, `isJinShen`, `getJiJie`, `getDaySpecialTypes`
 *
 * 依赖关系：
 * - 上游依赖：内部模块 `baziShenShaMap`、内部模块 `baziShenShaSetting`、内部模块 `baziJichuUtil`
